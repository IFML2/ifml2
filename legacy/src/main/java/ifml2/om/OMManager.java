...
public class OMManager {
    ...
    public static LoadStoryResult loadStoryFromFile(@NotNull String storyFileName, final boolean toInitItemsStartLoc,
        ...
        try {
            try {
                ...
                // LOG.debug("loadStoryFromFile :: File object for path \"{0}\" created",
    ...
    public static Library loadLibrary(String libPath) throws IFML2Exception {
        ...
        // --Loading from JAR--Reader reader = new BufferedReader(new
        // InputStreamReader(OMManager.class.getResourceAsStream(realRelativePath),
        // "UTF-8"));
    ...
    public static Library loadLibrary(@NotNull final File libFile) throws IFML2Exception {
        ...
        try {
            // todo remove LOG.debug("loadLibrary :: File object for path \"{0}\" created",
            ...
            Unmarshaller unmarshaller = context.createUnmarshaller();
            // todo: modify IDResolver for library hierarchical loading --
            // unmarshaller.setProperty(IDResolver.class.getName(), new IFMLIDResolver());

            ValidationEventCollector validationEventCollector = new ValidationEventCollector() {
                @Override
                public boolean handleEvent(ValidationEvent event) {
                    LOG.warn("There is ValidationEvent during unmarshalling of library {0}: {1}",
                            libFile.getAbsolutePath(), event);
                    return super.handleEvent(event);
                }
            };
            unmarshaller.setEventHandler(validationEventCollector);
                        ...
                        if (story != null) {
                            ...
                            for (Object object : story.getLibraries()) {
                                ...
                                if (!(object instanceof Library)) {
                                    throw new IFML2Exception("Member of getLibraries isn't a Library! It's {0}",
                                            object.getClass());
                                }
                                Library library = (Library) object;
                                LOG.debug("call() ::   - searching in lib {0}, class is {1}", library.getName(),
                                        aClass);
                                ...
                                // role definitions
                                    ...
                                    LOG.debug(
                                            "call() ::     it's nor Attribute, nor Action, nor RoleDefinition => nothing to search else");
                                }
                            }
                        } else {
                    ...
                    for (Object object : get(loweredName)) {
                        if (object != null) {
                                                                                                             ...
                                                                                                             // JAXB-546
                            {
                                if (aClass == Object.class) {
                                    LOG.warn(
                                            "getObjectOfClass() :: returns object \"{0}\" for \"{0}\" when aClass is Object!",
                                            object, name);
                                }
                                return object;
                            }
