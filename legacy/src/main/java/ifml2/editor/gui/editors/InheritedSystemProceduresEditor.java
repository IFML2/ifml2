...
public class InheritedSystemProceduresEditor extends AbstractEditor<InheritedSystemProcedures> {
    ...
    public InheritedSystemProceduresEditor(Window owner, InheritedSystemProcedures inheritedSystemProcedures,
        ...
        FilterList<Procedure> filteredProcedures = new FilterList<Procedure>(storyDataHelper.getProcedures(),
                new Matcher<Procedure>() {
                    @Override
        ...
        parseErrorHandlerCombo.setModel(new EventComboBoxModelWithNullElement<Procedure>(filteredProcedures,
                inheritedSystemProcedures.getParseErrorHandler()));

        // -- buttons --

        parseErrorHandlerOpenButton
                .setAction(new ButtonAction(parseErrorHandlerOpenButton, isParseErrorHandlerFilled()) {
                    @Override
                    public void init() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        if (procedure != null) {
                            ProcedureEditor procedureEditor = new ParseErrorHandlerProcedureEditor(procedure,
                                    storyDataHelper);
    ...
    private class ParseErrorHandlerProcedureEditor extends ProcedureEditor {
        ...
        protected void validateData() throws DataNotValidException {
            ...
            if (!hasParameter(CommonConstants.PARSE_ERROR_HANDLER_PRM_PHRASE)
                ...
                throw new DataNotValidException(MessageFormat.format("У процедуры должны быть параметры {0} и {1}.",
                        CommonConstants.PARSE_ERROR_HANDLER_PRM_PHRASE, CommonConstants.PARSE_ERROR_HANDLER_PRM_ERROR),
                        paramsEditForm);
            }
        }
    }
