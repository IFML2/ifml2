    ...
    private JMenuBar createMainMenu() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    JOptionPane.showMessageDialog(Editor.this,
                            "Ошибка во время сохранения истории: " + ex.getMessage());
                ...
                storyFileChooser.setFileFilter(new FileFilter() {
                    ...
                    public boolean accept(File file) {
                        return file.isDirectory()
                                || file.getName().toLowerCase().endsWith(CommonConstants.CIPHERED_STORY_EXTENSION)
                                || !file.exists();
                    }
                });
        ...
        storyMenu.add(new AbstractAction("Перехваты системных процедур...", GUIUtils.PREFERENCES_ICON) {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                InheritedSystemProceduresEditor inheritedSystemProceduresEditor = new InheritedSystemProceduresEditor(
                        Editor.this, inheritedSystemProcedures, story.getDataHelper());
        ...
        storyMenu.add(new AbstractAction("Запустить историю в Плеере...", GUIUtils.PLAY_ICON) {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    if (storyFile == null) {
                        JOptionPane.showMessageDialog(Editor.this,
                                "Прежде, чем запускать в Плеере, необходимо сохранить историю хотя бы один раз.",
            ...
            protected boolean beforeDelete(Procedure selectedElement) throws Exception {
                if (selectedElement != null) {
                    ...
                    if (affectedActionsList.size() > 0) {
                        ...
                        JOptionPane.showMessageDialog(Editor.this, message, "Процедура используется",
                    ...
                    if (selectedElement.equals(story.getInheritedSystemProcedures().getParseErrorHandler())) {
                        int answer = JOptionPane.showConfirmDialog(Editor.this,
                                "Эта процедура установлена как перекрывающая системный обработчик ошибки парсинга. Всё равно удалить?\n"
                                        + "В этом случае перекрытие будет отменено.",
                                "Процедура используется", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
                        return answer == JOptionPane.YES_OPTION;
                    }
